			...
			for(homo in c(FALSE, TRUE)){
				for(cj in cjs){
					...
					for(file in list.files(path)){
						if(grepl('alpha_summ',file)){
							alpha 		<- unlist(fread(paste0(path,file)))
							alpha_total <- cbind(alpha_total, alpha)
						}
						if(grepl('beta_pred_summ',file)){
							beta_pred 		<- unlist(fread(paste0(path,file)))
							beta_pred_total <- cbind(beta_pred_total, beta_pred)
						}
						if(grepl('g_pred_summ',file)){
							g_pred 			<- unlist(fread(paste0(path,file)))
							g_pred_total 	<- cbind(g_pred_total, g_pred)
						}
						if(grepl('theta_summ',file)){
							theta 		<- unlist(fread(paste0(path,file)))
							theta_total <- cbind(theta_total, theta)
						}
					}
